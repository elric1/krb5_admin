...
krb5_keyblock
krb5_v4_password(krb5_context ctx, char *passwd)
{
	krb5_data	pwd;
	...
	K5BAIL(krb5_c_string_to_key(ctx, 1, &pwd, &key_salt.data, &key));
...
krb5_error_code
hack_addrs(krb5_context ctx, krb5_auth_context actx)
{
	krb5_address	addr;
	char		addrc[4] = { 1, 1, 1, 1 };

	addr.addrtype = ADDRTYPE_INET;
	addr.length   = 4;
	addr.contents = addrc;

	return krb5_auth_con_setaddrs(ctx, actx, &addr, &addr);
}

void
krb5_rd_req_mine(krb5_context ctx, krb5_auth_context actx, krb5_data *in)
{
	krb5_error_code	ret;
	char		croakstr[2048] = "";

	K5BAIL(krb5_rd_req(ctx, &actx, in, NULL, NULL, 0, NULL));
...
krb5_error_code
krb5_rd_priv_mine(krb5_context ctx, krb5_auth_context actx, krb5_data *in,
	     krb5_data *out)
{
	krb5_replay_data	replay_data;

	return krb5_rd_priv(ctx, actx, in, out, &replay_data);
}

krb5_error_code
krb5_mk_priv_mine(krb5_context ctx, krb5_auth_context actx, krb5_data *in,
	     krb5_data *out)
{
	krb5_replay_data	replay_data;

	return krb5_mk_priv(ctx, actx, in, out, &replay_data);
}
